...
export default function Home() {
  ...
  const fetchPantry = useCallback(async () => {
    try {
      ...
      setError(
        e?.response?.data?.error || e?.message || "Failed to load pantry",
      );
      setPantry([]);
    } finally {
      setLoading(false);
      ...
      >
        ...
        {loading ? (
          <View style={{ paddingVertical: 24, alignItems: "center" }}>
            ...
            {!!error && (
              <Text style={{ marginTop: 8, color: theme.textDim }}>
                {error}
              </Text>
            )}
          </View>
          ...
          <>
            <View style={s.row}>
              ...
              <SummaryTile
                ...
                title="Expiring Soon"
                subtitle={
                  soonCount > 0 ? `${soonCount} in ≤ 7 days` : soonestLabel
                }
              />
            </View>
            ...
            <View style={s.row}>
              <SummaryTile
                icon="⚠️"
                title="Expired"
                subtitle={`${expiredCount} item${expiredCount === 1 ? "" : "s"}`}
              />
              <SummaryTile
                icon="📅"
                title="Soonest Expiry"
                subtitle={soonestLabel}
              />
            </View>
          </>
        )}
        ...
        {loading ? (
          ...
          <View style={s.activity}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[s.activitySub, { color: theme.primary }]}>
                Add items or edit pantry to see updates.
              </Text>
            </View>
          </View>
        ) : (
...
const makeStyles = (t: any) =>
  StyleSheet.create({
    ...
    activity: {
      ...
      marginBottom: 10,
      shadowColor: "#000",
      shadowOpacity: t.name === "light" ? 0.03 : 0.1,
      shadowRadius: 8,
      shadowOffset: { width: 0, height: 3 },
    },
    activityIconWrap: {
