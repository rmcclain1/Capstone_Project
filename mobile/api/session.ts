...
export type RailsSessionResponse = { ok: boolean; token?: string; user?: Record<string, any>; error?: string; status?: number };
...
export async function postToRails(firebaseIdToken: string): Promise<RailsSessionResponse> {
    ...
    const res = await fetch(url, { method: 'POST', headers: { 'Authorization': `Bearer ${firebaseIdToken}` } });
    const json = await res.json().catch(() => ({}));
    if (res.ok && json?.token) await saveRailsJwt(json.token);
    return { ok: res.ok, ...json, status: res.status };
}

export async function getRailsJwt(): Promise<string | null> { return readRailsJwt(); }
export async function clearRailsJwt(): Promise<void> { await wipeRailsJwt(); }
